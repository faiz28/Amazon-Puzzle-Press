    ...
    if puzzle_per_page>=60:
        for i in range(10):
            for j in range(6):
                ...
                upper_space = 0.25
                while(len_lower):
                    pdf.drawString((x-upper_space)*inch,(y-.2)*inch, str(xx[len_up-1]))
                    upper_space +=.2
                    len_lower -=1
                pdf.drawString((x-upper_space)*inch,(y-.2)*inch, "+")
                pdf.roundRect((x-.8)*inch,(y-.6)*inch,.9*inch,.8*inch,.1*inch, fill=False, stroke=True)
                print("done")
                pdf.line((x+.11)*inch,(y-0.25)*inch, (x-.8)*inch,(y-0.25)*inch)
